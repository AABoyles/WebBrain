...
export default {
  tag: 'weather',
  instruction: `WEATHER SKILL: To get current weather at a location, call <|tool_call>call:weather{input:<|"|>lat,lon<|"|>}<tool_call|> or <|tool_call>call:weather{input:<|"|>city name<|"|>}<tool_call|> (for city names, use with the Location tool to get coordinates first).
...
- Use <|tool_call>call:location{input:<|"|><|"|>}<tool_call|> first to get user's coordinates, then pass them here.`,
  async call(content) {
    ...
    if (!coordMatch) return 'Format: lat,lon (e.g. 40.71,-74.01). Use the Location tool first.';
