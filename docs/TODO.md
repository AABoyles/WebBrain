...
- [x] **MAC address** (`<mac>`) — OUI prefix → manufacturer; first-octet flags (unicast/multicast, locally administered)
...
- [x] **DNS record types** (`<dns>`) — explain what A, AAAA, CNAME, MX, TXT, SRV, CAA, SOA records do
- [x] **Unix signal** (`<signal>`) — `SIGTERM` ↔ `15`; explains default behavior and when each is used
- [x] **Exit code** (`<exit>`) — `137` → OOM-killed (128+SIGKILL); common shell and program exit codes
...
- [x] **Pigpen cipher** (`<pigpen>`) — Masonic cipher; encode to Unicode block characters approximating the grid symbols
...
- [x] **Semaphore** (`<semaphore>`) — flag semaphore letter descriptions as compass positions
