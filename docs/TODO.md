...
- [ ] **Tone generator** (`<tone>`) — play a pure sine wave at a given Hz or musical note; useful for tuning instruments, hearing tests, tinnitus masking
...
- [ ] **Dream log** (`<dream>`) — timestamped dream entries in a dedicated IndexedDB store; indexed by date for retrieval
...
- [ ] **VIN decoder** (`<vin>`) — World Manufacturer Identifier, model year encoding, plant code, check digit validation (NHTSA formula)
