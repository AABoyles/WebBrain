const DIAL = {
  ...
  TD:235,CL:56,CN:86,CO:57,KM:269,CD:243,CG:242,CR:506,HR:385,CU:53,CY:357,
  CZ:420,DK:45,DJ:253,DM:1,DO:1,EC:593,EG:20,SV:503,GQ:240,ER:291,EE:372,
  SZ:268,ET:251,FJ:679,FI:358,FR:33,GA:241,GM:220,GE:995,DE:49,GH:233,GR:30,
  GD:1,GT:502,GN:224,GW:245,GY:592,HT:509,HN:504,HU:36,IS:354,IN:91,ID:62,
  IR:98,IQ:964,IE:353,IL:972,IT:39,JM:1,JP:81,JO:962,KZ:7,KE:254,KI:686,
  ...
  NR:674,NP:977,NL:31,NZ:64,NI:505,NE:227,NG:234,MK:389,NO:47,OM:968,
  PK:92,PW:680,PS:970,PA:507,PG:675,PY:595,PE:51,PH:63,PL:48,PT:351,
  QA:974,RO:40,RU:7,RW:250,KN:1,LC:1,VC:1,WS:685,SM:378,ST:239,SA:966,
  ...
  UG:256,UA:380,AE:971,GB:44,US:1,UY:598,UZ:998,VU:678,VE:58,VN:84,
  YE:967,ZM:260,ZW:263,
};
const BY_CODE = {};
for (const [cc,dial] of Object.entries(DIAL)) {
  BY_CODE[dial] = BY_CODE[dial] ?? [];
  BY_CODE[dial].push(cc);
}
...
- "What country is +44?" → <dialcode>+44</dialcode>`,
  call(content) {
    ...
    if (!isNaN(num) && BY_CODE[num]) {
      return `+${num} → ${BY_CODE[num].join(', ')}`;
    }
    // Try as 2-letter ISO code
    const cc = input.toUpperCase();
    if (DIAL[cc]) return `${cc}: +${DIAL[cc]}`;
    return `Not found: "${content}". Try an ISO code (GB, FR) or dial code (+44, +33).`;
  },
  async handle() {},
};
